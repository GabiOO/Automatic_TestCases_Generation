...
is
   ...
   --Note that we only need to process the array of Conflicts, but anything else related to Conflict
   ...
   Conflicts : Conflicts_Type (1..3) := ( (North, South), (North, East), (West, South) ); --Given by the user
   ...
   procedure To_Red (L : in out Lights_Type; P : Path)
     with Pre => Safety_Property (L) and L (P) = Yellow,
          Post => Safety_Property( L ) and L = L'Old'Update ( P => Red);


   procedure To_Yellow (L : in out Lights_Type; P : Path)
     with Pre => Safety_Property (L) and L (P) = Green,
          Post => Safety_Property( L ) and L = L'Old'Update ( P => Yellow);
